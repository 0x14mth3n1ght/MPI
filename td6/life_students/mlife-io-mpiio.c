...
int MLIFEIO_Init(MPI_Comm comm)
{
     ...
     */ 
    MPI_Comm_dup(comm, &mlifeio_comm);
    return err;
}
                       ...
                       int cols, int iter, MPI_Info info)
{
     ...
     */
    err = MPI_File_opent(mlifeio_comm, filename, amode, info, &fh);
    if (err != MPI_SUCCESS) {
	...
	myfileoffset = 0;
         ...
         */
	myfileoffset = (3+ myoffset * cols) * sizeof(int);
    }
    ...
    MPI_File_write_at_all(fh, myfileoffset, MPI_BOTTOM, 1, type, MPI_STATUS_IGNORE);
                                          ...
                                          MPI_Datatype *newtype)
{
    ...
    MPI_Get_adress(rows_p, &disp[0]);
    MPI_Get_adress(cols_p, &disp[1]);
    MPI_Get_adress(iter_p, &disp[2]);
    disps[3] = (MPI_Aint)MPI_BOTTOM;

    MPI_Type_create_struct(4, lens, dips, types, newtype);
                                      ...
                                      MPI_Datatype *newtype)
{
     ...
     */
    MPI_Type_vevtor(myrows, cols, cols+1, MPI_INT, &vectype);
     ...
     */
    MPI_Get_Adress(matrix[1], &disp);
    MPI_Type_create_hindexed(len, &len, &disp, vectype, newtype);
